        ...
        <div className="graphType">
          ...
          <div className="graphTypeContents">
            <div className={type !== 'PIE' ? 'hide' : ''}>
              <DonutDigitalChart data={data.sylinder} shift="20" max="40" unit="mm" name="sylinder stroke" strokeColor="#8cb857" strokeColorLine="#2f4036"/>
              <DonutDigitalChart data={data.yawAngle} shift="15" max="30" unit="deg" name="yaw angle" strokeColor="#8cb857" strokeColorLine="#2f4036"/>
              <DonutDigitalChart data={data.aPort} max="500" unit="Bar" name="a port pressure" strokeColor="#3581c9" strokeColorLine="#16315b"/>
              <DonutDigitalChart data={data.bPort} max="500" unit="Bar" name="b port pressure" strokeColor="#3581c9" strokeColorLine="#16315b"/>
            </div>
            <div className={type != 'BAR' ? 'hide' : ''}>
              <DynamicBarChart data={data.sylinder} shift="20" max="40" unit="mm" name="sylinder stroke" barColor="#6f934f"/>
              <DynamicBarChart data={data.yawAngle} shift="15" max="30" unit="deg" name="yaw angle" barColor="#6f934f"/>
              <DynamicBarChart data={data.aPort} max="500" unit="Bar" name="a port pressure" barColor="#2c68a8"/>
              <DynamicBarChart data={data.bPort} max="500" unit="Bar" name="b port pressure" barColor="#2c68a8"/>
            </div>
            <div className={type != 'LINE' ? 'hide' : ''}>
              <DynamicLineChart data={data.sylinder} shift="20" max="40" unit="mm" name="lat. distance"/>
              <DynamicLineChart data={data.yawAngle} shift="15" max="30" unit="deg" name="yaw angle" />
              <DynamicLineChart data={data.aPort} max="500" unit="Bar" name="a port pressure" />
              <DynamicLineChart data={data.bPort} max="500" unit="Bar" name="b port pressure" />              
            </div>
